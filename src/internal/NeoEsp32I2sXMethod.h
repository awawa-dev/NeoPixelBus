...
public:
	...
	uint8_t i2sBus;
	uint8_t* pixelMuxData[8] = {nullptr};
	size_t pixelMuxDataLen[8] = {0};
	...
	size_t FillBuffers()
	{
		...
		uint8_t* lastDataPtr = i2sGetProcessedDataPtr(i2sBus);
		bool isFinished = i2sWriteDone(i2sBus);
		const size_t limit = 8 * 8;

		if (lastDataPtr != NULL && lastDataPtr > I2sBuffer + limit)
			lastDataPtr -= limit;
		...
		while (data1 < end1 || data2 < end2)
		{
			...
			while (!isFinished && pDma64 >= reinterpret_cast<uint64_t*>(lastDataPtr))
			{
				...
				xSemaphoreTake(i2sSemaphoreHandle(i2sBus), portMAX_DELAY);
				isFinished = i2sWriteDone(i2sBus);
				lastDataPtr = i2sGetProcessedDataPtr(i2sBus);
				if (lastDataPtr != NULL)
					lastDataPtr -= limit;
			}
		...
		return reinterpret_cast<uint8_t*>(pDma64) - I2sBuffer;
	}	
	...
	void Construct(const uint8_t busNumber, uint32_t i2sSampleRate, uint8_t muxId, uint8_t* data, size_t dataLen)
	{
		...
		if (I2sBuffer == nullptr)
		{
			...
			// 1 byte on the input => 8 (bits) * (bits of 4 bytes are used for neopixel) * only x8 on I2S1 is true 8bits mode and requires less memory than 16bits modes
			I2sBufferSize = MaxBusDataSize * 8 * 4 * ((busNumber == 1) ? 1 : 2);
...
public:
	...
	void StartWrite()
	{
		if (_context.IsAllMuxBusesUpdated())
		{
			...
			size_t dataSize = _context.FillBuffers();
			i2sWrite(_i2sBusNumber, _context.I2sBuffer, _context.I2sBufferSize, dataSize);
			_canCache = true;
		}
	}
